...
function oneTwoFiftyFive(){
    var arr= [];
    for(var i=1;i<=255;i++){
        arr.push(i);
    }return arr;
}
console.log (oneTwoFiftyFive())
...
function returnAllValue(numAll){
    var sum= 0;
...
console.log(returnAllValue([-5,2,5,12]))
...
function returnOddValues(range) {
    var arr = [];
    for (i=1; i<range;i++) {
        if (i % 2 ==1 ) {
            arr.push(i);
...
console.log(returnOddValues(100))
...
function swap(arr){
var first = arr[0];//arr[0]is 1
var last = arr[arr.length-1];// var last is [arr.length-1]= -2
arr[0]=last;//assigned arr[0] to [arr.length-1]which is -2
arr[arr.length-1]=first;//now the arr of [arr.length-1] is assigned to first, which is 1
